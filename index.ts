...
let appName : string = chalk.yellow.underline(` Welcome to "ATM-Machine": `);
console.log(`\n\t`,`\t`,`\t`,`${appName}\n`);

//----------------------------------- Main function ------------------------------------

let myBalance: string= `$350000`;
let balance: number = parseInt(myBalance.replace('$', ''));
let mypin = 1081;
...
const validatePIN = async (attempt: number): Promise<boolean> => {
...
const depositToAnotherAccount = async () => {
    ...
    let verMes : string = chalk.green(`Authentication Successful!`);
    console.log(`\n\t`,`\t`,`\t`,`${verMes}`);
    let usrBankAccName : string = chalk.yellow.italic(` Welcome "${userName}", you are in "${userName2}" ${bankNameAns.bank_name} Account:`);
    console.log(`\n\t`,`\t`,`${usrBankAccName}\n`); 
...
let usrAccName : string = chalk.yellow.italic(` ASSALAM O ALAIKUM "${userName}" Welcome to your Account: `);
    console.log(`\n\t`,`\t`,`${usrAccName}\n`); 
